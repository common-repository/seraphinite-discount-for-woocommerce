...
class Ui
{
	...
	static function InputBox( $type, $id, $value = null, $attrs = null, $addNames = false )
	{
		...
		if( $masked )
		{
			...
			$attrs[ 'onchange' ] = 'jQuery(this.parentNode).find("input[type=\\"hidden\\"]").val(seraph_wd.Ui._MaskEncode(this.value))';
	...
	static function Spinner( $big = false, array $attrs = null )
	{
		if( !$attrs )
			$attrs = array();
		Gen::SetArrField( $attrs, 'class.+', 'seraph_wd_spinner' . ( $big ? ' big' : '' ) );
	...
	static function ToggleButton( $cssSelectorItemToToggle, $attrsBtn = null, $attrs = null )
	{
		...
		Gen::SetArrField( $attrsBtn, 'onclick', 'seraph_wd.Ui._cb.ToggleButton_OnClick("' . Ui::EscHtml( $cssSelectorItemToToggle ) . '",this);return(false);' );
	...
	static function TokensList( $value, $id = null, $attrs = null, $addNames = false )
	{
		...
		Gen::SetArrField( $attrs, 'class.+', 'seraph_wd_TokensList seraph_wd_textarea rs' );
	...
	{
		Gen::SetArrField( $prms, 'btnsItemOperate.up.attrs.onclick', 'seraph_wd.Ui.ItemsList.MoveItem(\'' . $prms[ 'editorAreaCssPath' ] . '\',this,-1);return false;' );
		Gen::SetArrField( $prms, 'btnsItemOperate.down.attrs.onclick', 'seraph_wd.Ui.ItemsList.MoveItem(\'' . $prms[ 'editorAreaCssPath' ] . '\',this,1);return false;' );
		Gen::SetArrField( $prms, 'btnsItemOperate.del.attrs.onclick', 'seraph_wd.Ui.ItemsList.DelItem(\'' . $prms[ 'editorAreaCssPath' ] . '\',this);return false;' );
	...
	static function ItemsList_OperateBtnsTpl( array $prms, $attrs = null )
	{
		...
		$res .= Ui::Button( esc_html_x( 'AddItemBtn', 'admin.Common_ItemsList', 'seraphinite-discount-for-woocommerce' ), false, null, null, 'button', array_merge( $attrs, Gen::GetArrField( $prms, 'btnsOperate.add.attrs', array() ) ) );
		$res .= Ui::Button( esc_html_x( 'DelAllItemsBtn', 'admin.Common_ItemsList', 'seraphinite-discount-for-woocommerce' ), false, null, null, 'button', array_merge( $attrs, Gen::GetArrField( $prms, 'btnsOperate.delAll.attrs', array() ) ) );
	...
	{
		Gen::SetArrField( $prms, 'btnsOperate.add.attrs.onclick', 'seraph_wd.Ui.ItemsList.AddItem(\'' . $prms[ 'editorAreaCssPath' ] . '\',this);return false;' );
		Gen::SetArrField( $prms, 'btnsOperate.delAll.attrs.onclick', 'seraph_wd.Ui.ItemsList.DelAllItems(\'' . $prms[ 'editorAreaCssPath' ] . '\',this);return false;' );
		return( self::ItemsList_OperateBtnsTpl( $prms, $attrs ) );
	}

	static function ItemsList_NoItemsContent( $attrs = null )
	{
		return( Ui::Tag( 'span', esc_html_x( 'NoItemsInfo', 'admin.Common_ItemsList', 'seraphinite-discount-for-woocommerce' ), $attrs ) );
	}
	...
	{
		return( self::_MetaboxAdd( $id, $title, $callback, $callbacks_args, $screen, $context, $priority, array( 'seraph_wd', $classesAdd ), $classesRemove ) );
	}
	...
	static function PostBoxes( $title, $metaBoxes = array( 'body' => null ), array $callbacks = null, $callbacks_args = null, $blocksAttrs = null )
	{
		...
		Gen::SetArrField( $blocksAttrs, 'wrap.class.+', 'seraph_wd' );
		...
		echo( Ui::ViewInitContent( '.wrap.seraph_wd' ) );
	}

	static function ViewInitContent( $viewCssSelector )
	{
		return( Ui::ScriptInlineContent( 'document.addEventListener("DOMContentLoaded",function(){seraph_wd.Ui.Init(jQuery("' . $viewCssSelector . '").get(0));})' ) );
	}

	static function PostBoxes_Popup( $id, $title, $callback = null, $callbacks_args = null )
	{
		$boxId = 'seraph_wd_popup_' . $id;
		...
		<script>
			(function()
			{
				...
				closeBtn.on( "click", function(){ seraph_wd.Ui.PopupClose( "<?php echo( esc_attr( $id ) ); ?>" ); } );
			})();
		</script>
	...
	static function BannerMsg( $severity, $text, $opts = 0, $attrs = NULL )
	{
		...
		if( !( $opts & Ui::MsgOptDismissible ) )
			$class .= ' seraph_wd_dismiss_hidden';
		...
		$res .= '<div class="seraph_wd"><p class="content">' . $text . '</p></div>';
	...
	static function AdminBtnsBlock( $items, $mode )
	{
		...
		foreach( $items as $item )
		{
			...
			$prms = array();
			$linkParams = \apply_filters( 'seraph_wd_Ui_AdminBtnsBlock_Link', array( 'content' => null, 'attrs' => array( 'class' => array( 'dashicons', $item[ 'type' ] ) ) ), $item[ 'type' ] );
...
class UiPopups
{
	...
	static function Draw()
	{
		...
		foreach( self::$items as $id => $prms )
		{
			...
			$attrs[ 'id' ] = 'seraph_wd_popup_' . $id;
			$attrs[ 'style' ][ 'display' ] = 'none';
			$attrs[ 'attr-modal' ][ 'display' ] = $prms[ 'modal' ];
			$attrs[ 'attr-body' ] = rawurlencode( $body );

			Gen::SetArrField( $attrs, 'class.+', 'seraph_wd popup' );

			echo( Ui::Tag( 'div', null, $attrs ) );
		}

		if( $needModal )
			echo( Ui::Tag( 'div', null, array( 'class' => 'seraph_wd popup_modal_overlay', 'style' => array( 'display' => 'none' ) ) ) );
	}
}
