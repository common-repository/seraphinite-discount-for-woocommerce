...
class Net
{
	...
	static function GetHrFromWpRemoteGet( $requestRes, $soft = false, $smart = false )
	{
		...
		if( Gen::StrStartsWith( $errCode, 'seraph_wd:hr:0x' ) )
			return( 0xFFFFFFFF & intval( substr( $errCode, strlen( 'seraph_wd:hr:0x' ) ), 16 ) );
	...
	static function UrlDeParse( $urlComps, $flags = 0, $exclComps = array(  ), $inclComps = array() )
	{
		...
		$metas = array(
			...
			array( PHP_URL_QUERY,		'query',	array( '?',	'seraph_wd\\Net::UrlBuildQuery', '' ), array()	),
...
class Wp
{
	...
	static function GetTempFile( &$dirTmp = null )
	{
		...
		return( @tempnam( $dirTmp, substr( 'wd', 0, 3 ) ) );
	}
	...
	static private function _Wpml_SetCurLang( $lang )
	{
		...
		add_filter( 'home_url',
			function( $url )
			{
				...
				$_SERVER[ 'REQUEST_URI' ] = add_query_arg( array( '_seraph_wd_home_url_tmp_lang' => self::_Wpml_GetCurLang() ), $_SERVER[ 'REQUEST_URI' ] );
		...
		add_filter( 'home_url',
			function( $url )
			{
				...
				$_SERVER[ 'REQUEST_URI' ] = remove_query_arg( '_seraph_wd_home_url_tmp_lang', $_SERVER[ 'REQUEST_URI' ] );
				return( $url );
			}
		, 99999 );
	}
	...
	static function Loc_ScriptLoadEx( $handle, $domain, $ns, $pathAbsRoot )
	{
		...
		return( Wp::AddInlineScript( $handle, '(function(data){seraph_wd.Wp.Loc.SetData(data.locale_data.messages,"' . $domain . '");})(' . $translations . ')' ) );
	...
	static function Loc_ScriptLoad( $handle, $domain, $domainLocDir, $locSubPath )
	{
		...
		return( Wp::Loc_ScriptLoadEx( $handle, $domain, 'seraph_wd', $pathAbsRoot . '/' . $domainLocDir . '/' . $locSubPath ) );
	}
